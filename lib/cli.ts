...
import * as epubtools from './index.js';
...
// import { default as textStatistics } from 'text-statistics';
import glob from 'tiny-glob';

process.title = 'epubuilder';
program.version('0.8.6');
    ...
    .action(async (epubFN, unpackTo) => {
        try {
            ...
            await new Promise((resolve, reject) => {
                ...
                unzipExtractor.on('error', (err: any) => { reject(err); });
...
function printToc(toc: any, indent: number) {
    for (let item of toc) {
...
function mkindent(indent: number) {
    let ret = '';
